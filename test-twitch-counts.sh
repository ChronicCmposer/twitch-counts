#!/bin/sh
# test-twitch-counts.sh — verify the REAL twitch-counts binary
# (the full AArch64 port of twitch-counts.py).
#
# The binary under test is NOT built here.  It is resolved as
# "$BUILD/twitch-counts", where BUILD defaults to
...
# script via `make test`).  Run `make twitch-counts` first if it is
# missing.
#
...
# ---------------------------------------------------------------------------
# resolve the driver: $BUILD/twitch-counts, no in-script build.
# ---------------------------------------------------------------------------
tc_build_dir
BIN=$(tc_driver twitch-counts) || exit 2
tmpdir=$(tc_sandbox twitch-counts-test)
...
rc=$?
ok=0
[ "$rc" -eq 0 ] && grep -q "usage: twitch-counts" "$tmpdir/help.out" && ok=1
